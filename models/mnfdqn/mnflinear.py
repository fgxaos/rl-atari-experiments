...
# Custom libraries
from models.mnfdqn.norm_flows import MaskedNVPFlow
...
class MNFLinear(nn.Module):
    def __init__(
        ...
        self.r_zero_b1 = nn.Parameter(torch.Tensor(in_features))
        self.r_zero_b2 = nn.Parameter(torch.Tensor(in_features))
    ...
    def reset_parameters(self):
        ...
        self.r_zero_b1.data.normal_(0, in_stdv)
        self.r_zero_b2.data.normal_(0, in_stdv)
    ...
    def forward(self, input, kl=True):
        if self.training:
            ...
            weight_std = torch.clamp(
                torch.exp(self.weight_logstd), 0, self.threshold_var
            )
            bias_std = torch.clamp(
                torch.exp(0.5 * self.bias_logvar), 0, self.threshold_var
            )
            ...
            if not kl:
                ...
                mu_tilde = torch.mean(self.r_zero_b1.ger(cw), dim=1)
                neg_log_var_tilde = torch.mean(self.r_zero_b2.ger(cw), dim=1)
                ...
                logr += (
                    0.5
                    * (
                        -torch.exp(neg_log_var_tilde) * z_mu_square + neg_log_var_tilde
                    ).sum()
                )
